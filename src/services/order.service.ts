...
export const getOrderByOrderId = async (orderId: string): Promise<IOrderDocument> => {
  const order = (await OrderModel.aggregate([{ $match: { orderId } }])) as IOrderDocument[];
  return order[0];
};

export const getOrdersBySellerId = async (sellerId: string): Promise<IOrderDocument[]> => {
  const orders = (await OrderModel.aggregate([{ $match: { sellerId } }])) as IOrderDocument[];
  return orders;
};

export const getOrdersByBuyerId = async (buyerId: string): Promise<IOrderDocument[]> => {
  const orders = (await OrderModel.aggregate([{ $match: { buyerId } }])) as IOrderDocument[];
  return orders;
};
