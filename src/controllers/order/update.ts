...
import { BadRequestError, IDeliveredWork, uploads } from '@jahidhiron/jobber-shared';
...
export const requestExtension = async (req: Request, res: Response): Promise<void> => {
...
export const deliveryDate = async (req: Request, res: Response): Promise<void> => {
  const { error } = await Promise.resolve(orderUpdateSchema.validate(req.body));
  if (error?.details) {
...
export const buyerApproveOrder = async (req: Request, res: Response): Promise<void> => {
  const { orderId } = req.params;
